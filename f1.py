...
def plot_confusion_matrix(cm, savename, title='Confusion Matrix'):
    ...
    output_dir = '/home/yuanhua/xiangce/confusion3'
    os.makedirs(output_dir, exist_ok=True)
    outpath = os.path.join(output_dir, savename)
    plt.savefig(outpath, format='png')
    # plt.show()

all_predicts = np.load("predict.npy")
all_labels = np.load("label.npy")
for index in range(0, 20):
    ...
    plot_confusion_matrix(mat_norm, name + '_confusion_matrix.png', title=name + ' confusion matrix')
